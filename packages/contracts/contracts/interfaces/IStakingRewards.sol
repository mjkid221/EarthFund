...
interface IStakingRewards {
    ...
    /**
    address rewardToken
    uint256 lastRewardBalance // Used to track last known reward token balance of the contract. Current balance - last balance = amount to distribute
    mapping(address=>RewardDistribution) daoRewards
    mapping(address=>mapping(address=>UserStake)) userStakes
     */
    function rewardToken() external view returns (address rewardToken);

    function lastRewardBalance()
        external
        view
    ...
    function daoRewards(address _daoToken)
        external
        view
        returns (RewardDistribution memory daoRewards);

    function userStakes(address _daoToken, address _user)
        external
        view
        returns (UserStake memory userStake);
}
