...
describe("Thin Wallet", async () => {
  ...
  describe("Thin Wallet Transfer : Success cases", async () => {
    ...
    it("should handle multiple ERC20 token transfers", async () => {
      ...
      const userCInitialBalance : BigNumber = await EarthToken.balanceOf(userC.address);
      ...
      expect (await EarthToken.balanceOf(userC.address)).to.be.eq(userCInitialBalance.add(amountToTransfer.div(2)));
    ...
    it("should handle multiple ether transfers", async () => {
      expect (await ethers.provider.getBalance(userC.address)).to.be.eq(ethers.utils.parseEther("10000"));
      expect (await ethers.provider.getBalance(userB.address)).to.be.eq(ethers.utils.parseEther("10000"));

      const amountToTransfer : BigNumber = ethers.utils.parseEther("1000");
      const userCInitialBalance : BigNumber = await ethers.provider.getBalance(userC.address);
      ...
      expect (await ethers.provider.getBalance(userC.address)).to.be.eq(userCInitialBalance.add(amountToTransfer.div(2)));
