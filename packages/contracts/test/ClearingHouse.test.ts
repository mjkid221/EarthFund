...
describe.only("Clearing House", function () {
  ...
  describe("Creating Child DAO", () => {
    ...
    it("should revert when trying to register a child dao token contract and no governor is set", async () => {
      ...
      await expect(
        clearingHouse
          .connect(deployer)
          .registerChildDao(childDaoToken.address, false, 10000, 0)
    ...
    it("should revert when trying to register the 1Earth token contract", async () => {
      await expect(
        clearingHouse
          .connect(deployer)
          .registerChildDao(earthToken.address, false, 10000, 0)
    ...
    it("should be able to register child dao with autostaking set to true", async () => {
      ...
      await clearingHouse
        .connect(deployer)
        .registerChildDao(reflectiveTokenOne.address, true, 10000, 0);
    });
    ...
    it("should revert when trying to register child dao token contract that is not owned by the clearing house", async () => {
      ...
      await expect(
        clearingHouse
          .connect(deployer)
          .registerChildDao(reflectiveToken.address, false, 10000, 0)
  ...
  describe("Swap with reflective tokens", () => {
    ...
    it("should revert with 'child dao token burn error'", async () => {
      ...
      await clearingHouse
        .connect(deployer)
        .registerChildDao(reflectiveTokenOne.address, false, 10000, 0);
      await clearingHouse
        .connect(deployer)
        .registerChildDao(reflectiveTokenTwo.address, false, 10000, 0);
